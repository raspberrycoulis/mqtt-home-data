...
while (True):
  ...
  client.publish("sensors", "temperature,room=office-ms430,floor=upstairs value=" + "{:.1f}".format(air_data['T']))
  print("Humidity = {:.1f} %".format(air_data['H_pc']))
  client.publish("sensors", "humidity,room=office-ms430,floor=upstairs value=" + "{:.1f}".format(air_data['H_pc']))
  print("Pressure = " + str(air_data['P_Pa']/100) + " hPa")
  client.publish("sensors", "pressure,room=office-ms430,floor=upstairs value=" + str(air_data['P_Pa']/100))
  print("Lux = {:.2f} lux".format(light_data['illum_lux']))
  client.publish("sensors", "lux,room=office-ms430,floor=upstairs value=" + "{:.2f}".format(light_data['illum_lux']))
  print("Air quality index = {:.1f}".format(air_quality_data['AQI']))
  client.publish("sensors", "airquality,room=office-ms430,floor=upstairs value=" + "{:.1f}".format(air_quality_data['AQI']))
  print("Air quality accuracy = " + str(air_quality_data['AQI_accuracy']) + "/3")
  client.publish("sensors", "airquality-accuracy,room=office-ms430,floor=upstairs value=" + str(air_quality_data['AQI_accuracy']))
  print("Breath VOC = {:.2f} ppm".format(air_quality_data['bVOC']))
  client.publish("sensors", "bvoc,room=office-ms430,floor=upstairs value=" + "{:.2f}".format(air_quality_data['bVOC']))
  print("Estimated CO" + SUBSCRIPT_2 + " = {:.1f} ppm".format(air_quality_data['CO2e']))
  client.publish("sensors", "co2,room=office-ms430,floor=upstairs value=" + "{:.1f}".format(air_quality_data['CO2e']))
  print("Gas sensor resistance = " + str(air_data['G_ohm']) + " " + OHM_SYMBOL)
  client.publish("sensors", "gas-resistance,room=office-ms430,floor=upstairs value=" + str(air_data['G_ohm']))
  print("Peak amplitude = {:.2f} mPa".format(sound_data['peak_amp_mPa']))
  client.publish("sensors", "sound-peak-amp,room=office-ms430,floor=upstairs value=" + "{:.2f}".format(sound_data['peak_amp_mPa']))
  print("A-weighted sound pressure = {:.1f} dBA".format(sound_data['SPL_dBA']))
  client.publish("sensors", "sound-decibels,room=office-ms430,floor=upstairs value=" + "{:.1f}".format(sound_data['SPL_dBA']))
