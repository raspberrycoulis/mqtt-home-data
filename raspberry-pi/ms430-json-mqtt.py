...
while (True):
  try:
    ...
    if temperaure_data is not None and humidity_data is not None and pressue_data is not None and lux_data is not None and aq_index is not None and aq_accuracy is not None and bvoc_data is not None and co2_data is not None and gas_data is not None and peak_amp_data is not None and dba_data is not None:
          try:
            # Send data to MQTT
            now = datetime.datetime.now()
            client.publish(channel, JSON_mqtt_datam qos=1, retain=True)
            sys.stdout.flush()
          except Exception:
            # Error
